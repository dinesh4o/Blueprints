...
export default function IndicationMatrix({ clinicalData }: { clinicalData: any[] }) {
  ...
  if (data.length === 0) {
    return <div className="flex items-center justify-center h-full text-muted-foreground">Not enough data for matrix</div>;
  }
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-background/95 border border-border p-3 rounded-lg shadow-xl backdrop-blur-sm">
          ...
          <p className="text-sm text-muted-foreground">Max Phase: <span className="text-foreground font-medium">{data.maxPhase || 'N/A'}</span></p>
        </div>
      );
    }
    return null;
