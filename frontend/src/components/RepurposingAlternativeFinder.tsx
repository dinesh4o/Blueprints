...
const AnimatedMolecule3D = lazy(() => import("@/components/AnimatedMolecule3D"));
...
export function RepurposingAlternativeFinder({ initialCid, hideSearch }: { initialCid?: number, hideSearch?: boolean } = {}) {
  ...
  // Trigger auto-search when initialCid prop is provided (e.g. from ReportPage).
  // We read initialCid directly in the effect to avoid passing a stale handleSearch ref.
  const lastSearchedCid = useRef<number | undefined>(undefined);

  useEffect(() => {
    if (initialCid && initialCid !== lastSearchedCid.current) {
      lastSearchedCid.current = initialCid;
      handleSearch(initialCid.toString());
    }
  // handleSearch is stable for the duration of this component instance.
  ...
  return (
    <div className="space-y-6">
                  ...
                  <Card key={alt.cid} className={`border bg-[#121214] flex flex-col ${isTwin ? 'border-zinc-500' : 'border-[#27272a]'}`}>
                    <CardHeader className="pb-3 px-4 pt-4 border-b border-[#27272a] bg-[#09090b]">
                      <div className="flex justify-between items-start mb-1">
                        ...
                        <Badge variant="outline" className="bg-zinc-900 border-zinc-700 text-zinc-300 flex-shrink-0">
                          Score: {alt.structuralEvidenceScore.toFixed(1)}
                        </Badge>
                      </div>
                    ...
                    <CardContent className="p-0 flex-1 flex flex-col overflow-hidden">
                      <div className="h-[200px] w-full bg-zinc-950/50 relative border-b border-[#27272a]">
                          <Suspense fallback={<div className="h-[200px] flex items-center justify-center text-zinc-500 text-sm">Loading 3D...</div>}>
      ...
      {searched && !loading && alternatives.length === 0 && (
         <Card className="border-[#27272a] bg-[#121214]">
            <CardContent className="pt-6">
              <p className="text-center text-zinc-500 italic">No compelling structural analogs or repurposing candidates found.</p>
            </CardContent>
          </Card>
      )}
