...
export function RepurposingVerdict({
  ...
  const { verdict, verdictColor, verdictIcon, verdictBg, signals, strengths, risks } = useMemo(() => {
    ...
    const clinicalScore = Math.min((clinicalData.length * 0.06) + (phase34 * 0.15) + (recruiting * 0.05), 1);
    signals.push({
      label: 'Clinical Evidence',
      icon: <FlaskConical className="w-3.5 h-3.5" />,
      value: clinicalScore,
      detail: `${clinicalData.length} trials${phase34 ? `, ${phase34} Phase 3/4` : ''}`,
      tag: clinicalScore >= 0.4 ? 'strength' : 'risk',
    });

    // Literature density
    const litScore = Math.min(literatureData.length * 0.06, 1);
    signals.push({
      label: 'Literature Support',
      icon: <FileText className="w-3.5 h-3.5" />,
      value: litScore,
      detail: `${literatureData.length} publications`,
      tag: litScore >= 0.3 ? 'strength' : 'risk',
    });
    ...
    // IP freedom
    const ipScore = patentData.length === 0 ? 0.9 : Math.max(0.1, 0.6 - patentData.length * 0.15);
    signals.push({
      label: 'IP Freedom',
